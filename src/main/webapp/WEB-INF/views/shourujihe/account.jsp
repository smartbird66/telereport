                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-4">
                                        <select class="form-control" name="productCode" id="productCode">
                                            <%@taglib  uri="http://java.sun.com/jsp/jstl/core" prefix="b" %>
                                            <b:if test="${not empty rpAccountFeeRecordForm.productCode}">
                                                <b:forEach items="${productList}" var="productList">
                                                    <b:if test="${rpAccountFeeRecordForm.productCode==productList.productCode}">
                                                        <option value="${rpAccountFeeRecordForm.productCode}">${productList.productName}</option>
                                                    </b:if>
                                                </b:forEach>
                                            </b:if>
                                            <%@taglib  uri="http://java.sun.com/jsp/jstl/core" prefix="b" %>
                                            <b:if test="${empty rpAccountFeeRecordForm.productCode}">
                                                <option value="">全部</option>
                                            </b:if>
                                    ...
                                    <div class="col-sm-4">
                                        <select class="form-control" name="checkStatus" id="checkStatus">
                                            <%@taglib  uri="http://java.sun.com/jsp/jstl/core" prefix="b" %>
                                            <b:if test="${not empty rpAccountFeeRecordForm.checkStatus}">
                                                <b:if test="${rpAccountFeeRecordForm.checkStatus==0}">
                                                    <option value="0">未稽核</option>
                                                </b:if>

                                                <b:if test="${rpAccountFeeRecordForm.checkStatus==1}">
                                                    <option value="1">稽核成功</option>
                                                </b:if>

                                                <b:if test="${rpAccountFeeRecordForm.checkStatus==2}">
                                                    <option value="2">稽核失败</option>
                                                </b:if>
                                            </b:if>
                                            <%@taglib  uri="http://java.sun.com/jsp/jstl/core" prefix="b" %>
                                            <b:if test="${empty rpAccountFeeRecordForm.checkStatus}">
                                                <option value="">全部</option>
                                            </b:if>
                                            <option value="">全部</option>
                                            <option value="0">未稽核</option>
                                    ...
                                    <div class="col-sm-4">
                                        <select class="form-control" name="accountFeeTypeCode" id="accountFeeTypeCode">
                                            <%@taglib  uri="http://java.sun.com/jsp/jstl/core" prefix="b" %>
                                            <b:if test="${not empty rpAccountFeeRecordForm.accountFeeTypeCode}">
                                                <b:forEach items="${rpAccountTypeCodeTList}" var="rpAccountTypeCodeTList">
                                                    <b:if test="${rpAccountFeeRecordForm.accountFeeTypeCode==rpAccountTypeCodeTList.accountTypeName}">
                                                        <option value="${rpAccountFeeRecordForm.accountFeeTypeCode}">${rpAccountTypeCodeTList.accountTypeName}</option>
                                                    </b:if>
                                                </b:forEach>
                                            </b:if>
                                            <%@taglib  uri="http://java.sun.com/jsp/jstl/core" prefix="b" %>
                                            <b:if test="${empty rpAccountFeeRecordForm.accountFeeTypeCode}">
                                                <option value="">全部</option>
                                            </b:if>
                                            <%@taglib  uri="http://java.sun.com/jsp/jstl/core" prefix="city" %>
                                            <city:forEach items="${rpAccountTypeCodeTList}" var="rpAccountTypeCodeTList" >
                            ...
                            <table class="table table-bordered table-hover table-striped">
                                ...
                                <b:forEach items="${PageInfo.list}" var="accountFeeRecord">
                                    <tr>
                                        ...
                                        <td colspan="2"><a onclick="fail(${accountFeeRecord.id})">稽核失败</a></td>
                                    </tr>
                                </b:forEach>
                            </table>
...
<script>
    ...
    function fail(Aid) {
        var httpRequest = new XMLHttpRequest();//第一步：建立所需的对象
        httpRequest.open('GET', '${pageContext.request.contextPath}/shourujihe/account/update?Aid='+Aid+'&Cid=2', true);//第二步：打开连接  将请求参数写在url中  ps:"./Ptest.php?name=test&nameone=testone"
        httpRequest.send();//第三步：发送请求  将请求参数写在URL中
        document.getElementById('formId').action="${pageContext.request.contextPath}/shourujihe/account"
        document.getElementById('formId').submit();
        console.log("重新提交表单")
